...
namespace EzPay.ModelUpdater
{
    ...
    public static class Program
    {
        ...
        static void InitialDataAzure()
        {
            DbContextOptionsBuilder options = new DbContextOptionsBuilder();
            options.UseSqlServer(
                "Server=tcp:ezpay.database.windows.net,1433;Initial Catalog=ezpay;Persist Security Info=False;User ID=qualco2;Password=7ioeQ335i58b;MultipleActiveResultSets=False;Encrypt=True;TrustServerCertificate=False;Connection Timeout=30;");
            EzPaySqlServerContext ctx = new EzPaySqlServerContext(options.Options);
            CheckContext(ctx);
            AddSettlementTypes(ctx);
            var import = new Importer();
            var data = import.GetResults();
            ...
            foreach (var debt in data)
            {
                ...
                c.NormalizedEmail = c.Email.ToUpper();
                c.NormalizedUserName = c.UserName;
                c.SecurityStamp = Guid.NewGuid().ToString().ToLower();
                c.ConcurrencyStamp = Guid.NewGuid().ToString().ToLower();
                c.PasswordHash = PasswordHash.HashPassword("123");
                ctx.Add(c);
                ctx.AddRange(debt.Value);
                processed++;
            }
